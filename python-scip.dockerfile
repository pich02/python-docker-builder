FROM pich02/python3-glibc2.24:3.13.12
...
RUN export MAKEFLAGS="-j$(grep -c ^processor /proc/cpuinfo)"; \
    OPENSSL_ROOT_DIR=/openssl-3.2.0/ python3.14 -m pip install --no-input --extra-index-url=${PIP_EXTRA_INDEX} cmake; \
    if [ "${TARGETARCH}${TARGETVARIANT}" != "armv7" ]; then \
      OPENSSL_ROOT_DIR=/openssl-3.2.0/ python3.14 -m pip install --no-input --extra-index-url=${PIP_EXTRA_INDEX} ninja; \
    fi
...
RUN wget --no-check-certificate https://scipopt.org/download/release/scip-9.1.1.tgz; \
    ...
    python3.14 -m cmake .. -DPAPILO=off -DZIMPL=off -DIPOPT=off; \
    make -j$(grep -c ^processor /proc/cpuinfo); \
    make install

RUN python3.14 -m pip install --no-input --extra-index-url=${PIP_EXTRA_INDEX} pyscipopt==5.3.0; \
  python3.14 -c "import pyscipopt;"
